...
const { site, width, height, totalMs, cues } = JSON.parse(
...
const doc = `# Prompt — narrate & stitch \`divergence-demo.mp4\`
...
- \`divergence-demo.mp4\` — a silent screen recording, total **${cue(
  totalMs
)}** (no audio track), ${width}×${height}, 30 fps, H.264. Inspect it to confirm.
- The numbered narration lines in **Narration** below. Each has a start cue
  in \`m:ss.s\` measured from the very start of the video.

The recording is a real browser session against ${site} — not a mock-up.
